...
Template.Project_Page.onCreated(function onCreated() {
  this.autorun(() => {
    this.subscribe('Projects');
  });
  this.messageFlags = new ReactiveDict();
  this.messageFlags.set(displayErrorMessages, false);
  this.context = ProjectSchema.namedContext('Project_Page');
});

Template.Project_Page.helpers({
  projectDataField(fieldName) {
    const projectData = Projects.findOne(FlowRouter.getParam('_id'));
    // See https://dweldon.silvrback.com/guards to understand '&&' in next line.
    return projectData && projectData[fieldName];
  },
  errorClass() {
